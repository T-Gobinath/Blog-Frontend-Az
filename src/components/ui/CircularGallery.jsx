...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  // Render at an extremely high fixed resolution to guarantee crystal 3D clarity
  const renderFontSize = 64; 
  const renderFont = `bold ${renderFontSize}px Figtree, sans-serif`;
  context.font = renderFont;
  
  const lines = text.split('\\n');
  const lineHeight = Math.ceil(renderFontSize * 1.25);
  ...
  // Tight padding guarantees maximum texture space usage inside WebGL limits
  const paddingX = 40; 
  const paddingY = 20; 
  
  canvas.width = Math.ceil(maxLineWidth) + paddingX; 
  canvas.height = (lineHeight * lines.length) + paddingY; 
  ...
  // High contrast drop-shadow to pop mathematically off dark backgrounds
  ...
  // Return true texture aspect ratio derived from intrinsic canvas bounds
  return { 
    texture, 
...
class Title {
  ...
  createMesh() {
    const { texture, aspect } = createTextTexture(this.gl, this.text, this.font, this.textColor);
    this.aspect = aspect;
    ...
    this.mesh.position.z = 0.01; // Avoid any potential z-fighting bounds
...
class Media {
  constructor({
    ...
    this.bend = bend;
    this.baseBend = bend; // Save original bend for responsive resizing
    this.textColor = textColor;
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Removed wobble effect per request
          p.z = 0.0;
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Scale bend dynamically: mobile screens cannot handle aggressive viewport curving
    this.bend = isMobile ? this.baseBend * 0.4 : this.baseBend;
    ...
    this.padding = isMobile ? 1.0 : 2.0; // Draw cards slightly closer together on tiny screens
